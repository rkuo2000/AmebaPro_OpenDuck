    ...
    FootTarget &foot)
{
    ...
    if (phase >= GAIT_SWING_START && phase < GAIT_SWING_END) {
        ...
        // Original walk engine: zero-slope cubic rise, a peak-height hold,
        // then zero-slope descent. Keep clearance during forward transfer
        // instead of reaching full height only at a single instant.
        const float riseFraction = (1.0f - GAIT_LIFT_HOLD_FRACTION) * 0.5f;
        const float liftPhase = clampf(
            (s < 0.5f ? s : 1.0f - s) / riseFraction, 0.0f, 1.0f);
        foot.z = z0 + stepHeight * smoothStep(liftPhase);
    } else {
...
void generateFeet(float gaitPhase, FootTarget &left, FootTarget &right)
{
    ...
    footTrajectory(phase, GAIT_STEP_LENGTH, GAIT_STEP_HEIGHT, GAIT_STANCE_Z, left);
    footTrajectory(fmodf(phase + 0.5f, 1.0f),
                   GAIT_STEP_LENGTH, GAIT_STEP_HEIGHT, GAIT_STANCE_Z, right);
