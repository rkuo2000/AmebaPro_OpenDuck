...
AMB82Mini::AMB82Mini()
    : imu_(), smsSts_(0), gaitPhase_(0.0f), lastControlUs_(0),
      ...
      commandedTicks_{}, poseCommanded_(false)
{
}
...
bool AMB82Mini::writePose(const float *angles, uint16_t speed, uint8_t acceleration)
{
    ...
    for (int i = 0; i < NUM_JOINTS; ++i) commandedTicks_[i] = positions[i];
    poseCommanded_ = true;
    return true;
}

bool AMB82Mini::readJointFeedback(uint8_t jointIndex, int &target, int &actual)
{
    target = -1;
    actual = -1;
    if (!poseCommanded_ || jointIndex >= NUM_JOINTS) return false;
    target = commandedTicks_[jointIndex];
    actual = smsSts_.ReadPos(jointCalibration[jointIndex].id);
    return actual >= 0 && actual <= 4095;
}
